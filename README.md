...
├── cloudfunctions/    云函数（snkg-coze / snkg-kg / snkg-sn / snkg-health / snkg-login / snkg-user 等）
├── scripts/           预计算与数据导入脚本
├── data/              预计算生成的 JSON 数据
├── doc/               需求文档与原始数据
...
2. 非密钥配置已随仓库提供（`cloudfunctions/snkg-coze/config.js`），`WORKFLOW_ID` / `SPACE_ID` 已核对原线上值。Coze Key 唯一编辑源为 `ResearchBridge/private/coze/credentials.json`；普通整包上传前在 ResearchBridge 运行 `node scripts/rotate_coze_credentials.js --prepare-local`（只把密钥生成到本仓库 `cloudfunctions/snkg-coze/coze-credentials.json`，0600、gitignored，绝不触云）；日常云端密钥轮换仍运行 `--apply` 和 `--verify`。
3. `cloudbaserc.json` 保留真实运行参数；本地目录已与部署名同名（`snkg-*`），CLI 直接 `tcb fn deploy snkg-user --envId yuelai-0gawhvuc757cd498`，无需 `--dir`。开发者工具中云开发环境选择 `yuelai` 后，右键 `cloudfunctions/` 下的 `snkg-*` 目录上传部署即可；不要直接 `--all`，也不要按旧目录名发布无前缀函数。
4. 首次或全量更新 `snkg-coze` 包前，先运行上面的 `--prepare-local` 确保包内有私有密钥文件（`cloudfunctions/snkg-coze/coze-credentials.json`）。复制品不作为编辑源、不提交Git。
...
**服务端（云函数）**
- 所有函数部署名为 `snkg-<原函数名>`，本地目录已与部署名同名；部署名与运行参数见 `cloudbaserc.json`，CLI 部署无需 `--dir`。`path` 表示 HTTP 访问服务，不用于目录映射。
...
- `coze-credentials.json`、`project.private.config.json`、`.env` 被 Git 忽略；非秘密的 `cloudfunctions/snkg-coze/config.js` 跟随版本管理。
